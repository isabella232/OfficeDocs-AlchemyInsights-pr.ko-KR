---
title: 9000240 Outlook.com 답장
ms.author: daeite
author: daeite
manager: joallard
ms.date: 04/21/2020
ms.audience: Admin
ms.topic: article
ms.service: o365-administration
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.custom:
- "1825"
- "9000240"
ms.openlocfilehash: 3e3dd79cc2f03da9b0fa98f8f65ab6e6f208438bff8b3d3318529a93de52b7fc
ms.sourcegitcommit: b5f7da89a650d2915dc652449623c78be6247175
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 08/05/2021
ms.locfileid: "53961669"
---
# <a name="replying-in-outlookcom"></a>Outlook.com에서 회신

한 번씩 한 메시지에 회신:

1. 메시지 목록에서 회신할 메시지를 선택합니다.
2. At the top right of the message pane, select <img src='data:image/png;base64,' /> **회신 또는** <img src='data:image/png;base64,' /> **전체 회신 .**
3. 메시지를 입력한 다음 보내기 를 **선택합니다.**

모든 메시지에 자동으로 회신:

1. 이제  <img src='data:image/png;base64,' />
**설정**  >  **모든 Outlook 설정**  >  **메일**  >  **자동 응답을 사용하여** 자동 응답 설정 [을 열 수 있습니다.](https://outlook.live.com/mail/options/mail/automaticReplies)
2. **자동 회신 켜기** 토글을 선택합니다.
3. 특정 **기간** 동안만 답장 보내기 확인란을 선택하면 다음을 할 수 있습니다.
    - 선택할 때만 답장 보내기. 기간을 설정하지 않은 경우 자동 회신은 끄기 전까지 계속 설정됩니다.
    - 일정 차단
    - 자동으로 새 초대 거부
    - 이 기간 동안 회의를 거절 및 취소합니다.
4. 부재 중일 때 메시지 상자에 다른 사람이 보낼 메시지를 입력합니다.
5. 연락처로만 답장하려면 연락처에 답장만 보내기 **확인란을** 선택합니다.
6. **저장** 을 선택합니다.

Outlook.com에서 자동 [전달에 대해 더 Outlook.](https://support.office.com/article/14614626-9855-48dc-a986-dec81d07b1a0?wt.mc_id=Office_Outlook_com_Alchemy)