---
title: 다음에서 서명 웹용 Outlook
ms.author: daeite
author: daeite
manager: joallard
ms.date: 04/21/2020
ms.audience: Admin
ms.topic: article
ms.service: o365-administration
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.custom: ''
ms.openlocfilehash: 550b1025213ae54c17aed9ae5829896d2c83a6196b3251bbd80a36e14e790b45
ms.sourcegitcommit: b5f7da89a650d2915dc652449623c78be6247175
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 08/05/2021
ms.locfileid: "54040944"
---
# <a name="create-an-email-signature-in-outlook-on-the-web"></a>전자 메일 주소에서 전자 메일 서명 웹용 Outlook

1. 이제  <img src='data:image/png;base64,' />
 **설정**  >  **모든 Outlook 보기**  >  **을 작성하고 회신합니다.**
1. 전자 **메일 서명에서** 서명을 입력하고 사용 가능한 서식 옵션을 사용하여 모양을 변경합니다.

*참고:* 계정당 하나의 서명만 사용할 수 있습니다.

자세한 내용은 에서 전자 메일 서명 만들기 및 [추가를 웹용 Outlook.](https://support.office.com/article/5ff9dcfd-d3f1-447b-b2e9-39f91b074ea3)
