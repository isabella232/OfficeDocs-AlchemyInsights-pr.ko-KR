...
---
# <a name="associate-existing-site-with-a-hub-site"></a>기존 사이트를 허브 사이트에 연결

기존 사이트를 허브 사이트에 연결 하려면 다음을 수행 합니다.
  
1. 사이트에서 **설정** (오른쪽 위 모서리에 있는 기어 아이콘)을 선택한 다음 **사이트 정보**를 선택 합니다.

2. **사이트 정보 편집** 패널의 **허브 사이트 연결** 메뉴에서 사이트와 연결할 허브 사이트를 선택 하 고 **저장**을 클릭 합니다. 사이트의 디자인이 허브 사이트의 모양과 일치 하도록 변경 되 고 위쪽에 허브 사이트 탐색 모음이 표시 됩니다.
